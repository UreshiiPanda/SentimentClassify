...
for prune_num in {1..5}; do
    ...
    printf "\n\n"
    printf "Embeddings Training data with $((prune_num - 1))-count words pruned:"
    ...
    python3 train.py train"$prune_num".txt dev.txt test.txt
    # compare training results for other classifiers
    # comment out the MLP in classify.py for faster runtimes
    python3 classify_embs.py train"$prune_num".txt dev.txt test.txt
done


# run the best-performing model on the test data
python3 mlp.py train.txt test.txt > test.txt.predicted 
